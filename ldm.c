...
char *
js_get_str(char *buf, jsmntok_t *t) {
...
handle_ipc_event (int so)
{
	...
	// Parse the json
	jsmn_init(&p);
	r = jsmn_parse(&p, buf, r, t, 10);

	// Basic error checking
	if (r < 0 || t[0].type != JSMN_OBJECT || t[1].type != JSMN_STRING) {
		printf("Malformed ipc request\n");
		writes(so, "{\"err\":\"Malformed request\"}");
		return 0;
	}

	// The first token is always the action to perform
	act = js_get_str(buf, &t[1]);

	if (!strcmp(act, "remove")) {
		char *path = js_get_str(buf, &t[2]);

		if (path) {
			strip_slash(path);

			dev = device_search(path);

			if (dev && device_is_mounted(dev->udev)) {
				device_unmount(dev->udev) ?
					writes(so, "{\"ret\":\"ok\"}"):
					writes(so, "{\"err\":\"Could not unmount the device\"}");
				return 1;
			}
		}

		writes(so, "{\"err\":\"No such device\"}");

		return 0;
	}
	else if (!strcmp(act, "list")) {
		writes(so, "{\"ret\":[");
		TAILQ_FOREACH(dev, &g_devices, ptr) {
			if (1) {
				writes(so, "\"");
				writes(so, dev->rnode);
				writes(so, "\"");
				if (dev != TAILQ_LAST(&g_devices, dl_head))
					writes(so, ",");
			}
		}
		writes(so, "]}");
...
main (int argc, char *argv[])
{
	...
	while ((opt = getopt(argc, argv, "hdg:u:p:c:V")) != -1) {
		switch (opt) {
			case 'd':
				daemon = 1;
				break;
	...
	ipcfd = socket(PF_UNIX, SOCK_STREAM, 0);
	...
	strcpy(so.sun_path + 1, SOCK_PATH);

	// Beware of the tricky length field, the length must be calculated correctly otherwise garbage
	// (or nulls) will be appended to the socket name, causing a lot of headaches to clients when
	// they try to connect to us
	// https://bugzilla.kernel.org/show_bug.cgi?id=19812
	if (bind(ipcfd, (struct sockaddr *)&so,
			offsetof(struct sockaddr_un, sun_path) + strlen(SOCK_PATH) + 1) < 0) {
		perror("bind");
		return EXIT_FAILURE;
	}
	if (listen(ipcfd, 1) < 0) {
	...
	while (g_running) {
		...
		// ipc message on the socket
		if (pollfd[3].revents & POLLIN) {
			struct sockaddr_un a;
			socklen_t a_len;
			int c;

			c = accept(ipcfd, (struct sockaddr *) &a, &a_len);

			while (handle_ipc_event(c))
				;
